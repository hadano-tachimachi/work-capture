...
| [Work_Capture_Session_Retrospective_2026-07-01.md](./Work_Capture_Session_Retrospective_2026-07-01.md) | **2026-07-01 実装セッション振り返り**（本番デプロイ・UI改善・デプロイ障害） |
...
| AI（本番） | Gemini API（`gemini-3.1-flash-lite`）ほか OpenAI / Claude 切替可 |
...
| 5 | DB 保存 | ✅ |
| 6 | Inbox Zero | ✅ |
| 7 | Dashboard | ✅ MVP（PC 2 ペイン + SP 今日やること） |

### Phase 7 — Dashboard（MVP 実装済）

| デバイス | 画面 | 役割 |
|---------|------|------|
| PC | `/dashboard` 2 ペイン | 実行の主戦場（一覧 + 詳細 + 完了） |
| SP | `/capture/tasks` | 補助（外出先で 1 件完了） |

詳細ワイヤー・API・チェックリストは [Work_Capture_UI_Design.md](./Work_Capture_UI_Design.md) §3-2, §2-9, §10。
...
| 機能 | 理由 |
|------|------|
| Dashboard 3 ペイン / カレンダー / アクションプラン UI | Phase 7 MVP は 2 ペイン + 完了のみ |
| 認証・マルチユーザー | 個人 PoC |
| 音声ファイル永続ストレージ | transcript 優先 |
| ボトムタブ / SP タスク管理の主役化 | Capture 優先の設計思想 |
| PWA / ネイティブ | Web のみで開始 |
...
| 2026-07-01 | Dashboard MVP 設計（PC 2 ペイン + SP 今日やること） |
